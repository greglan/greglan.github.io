...
* Expression of $$\bke{\psi_1}{M}{\psi_2}$$ with a trace operator [2, p209]:

  $$ \bke{\psi_1}{M}{\psi_2} = \tr(\ket{\psi_2} \bra{\psi_1}M)$$ 
...
* Density operator/matrix of the system of an ensemble of pure states [1, p99]: 
...
* Unitary evolution of a density operator [1, p99]: 
  
  $$\rho \xrightarrow[]{U} U \rho U^\dagger = \sum_i p_i U\ket{\psi_i} \bra{\psi_i} U^\dagger$$ 
...
* Probability of getting result $$m$$ if the initial state was $$\ket{\psi_i}$$ [1, p99]: 
  
  $$p(m \vert i) = \bke{\psi_i}{M_m^\dagger M_m}{\psi_i} = \tr \left(M_m^\dagger M_m \ket{\psi_i} \bra{\psi_i} \right)$$ 
* Probability of getting result $$m$$ [1, p99]: 
  
  $$p(m) = \sum_i p(m \vert i)p_i = \tr (M_m^\dagger M_m \rho )$$
* State after getting result $$m$$ [1, p99]: 
  
  $$\ket{\psi_i^m} = \frac{M_m \ket{\psi_i}}{\sqrt{\left\vert \bke{\psi_i}{M_m^\dagger M_m}{\psi_i} \right\vert^2}}$$
* Density operator of the ensemble of state after getting result $$m$$ [1, p100]: 
  
  $$\rho_m = \frac{M_m \rho M_m^\dagger}{\tr ( M_m^\dagger M_m \rho )}$$
